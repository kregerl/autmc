use crate::{loader::{Library, download_java_version_manifest, download_java_runtime_manifest}, downloader::{download_all}};

use super::*;

use std::{fs::{File, self}, env, io::BufReader, time::Instant};
use chrono::{Timelike};
...
fn test_log_purge() {
    ...
    for second in 0..=59 {
        ...
        let datetime_str = format!("launcher_log_{}.log", datetime.format("%Y-%m-%dT%H:%M:%S").to_string());
    ...
    let mut dir_entries = file_paths.filter_map(|path| 
        path.ok()
    ).collect::<Vec<_>>();
    ...
    let result: Vec<String> = dir_entries
        .iter()
        .map(|entry| entry.file_name().to_str().unwrap().into()).collect();
...
fn test_downloader() {
    ...
    assert!(var.is_ok());
    let path = Path::new(&var.unwrap()).join("resources").join("libraries_test_1.19.2.json");
    ...
    assert!(result.is_ok());
    println!("Successfully downloaded libraries in {}ms", start.elapsed().as_millis());
}
